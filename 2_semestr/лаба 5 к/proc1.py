from math import sqrt,sin,cos
from mGraph import *
#Программа для поиска корней функции и уточнения их


def f(x):
    #Функция обработки
    if x == 0:
        y = 1
    else:
        y = sin(x)
    return y


def zoloto(x1 = 0, x2 = 0, eps = 0,interMax = 0,inter = 0):
    """Возвращает корень и кол-во итераций."""
    #Проверяет являются ли границы отрезков корнями
    if f(x1) == 0 or f(x2) == 0:
        if f(x1) == 0:
            return x1,0
        if f(x2) == 0:
            return x2,0
    else:
        t = (sqrt(5) - 1) / 2
        t = round(t,6)
        x3 = x1 + (1 - t) * (x2 - x1)
        x4 = x1 + t * (x2 - x1)
        #Проверяет есть ли в функции корни
        if not((f(x1) > 0) and (f(x2) > 0) or (f(x1) < 0) and (f(x2) < 0)):
            if abs(x1-x2) > eps:
                if inter < interMax:
                    if abs(f(x3)) > abs(f(x4)):
                        return zoloto(x3,x2,eps,interMax,inter + 1)
                    else:
                        return zoloto(x1,x4,eps,interMax,inter + 1)
                else:
                    return x3,inter
        
            x3 = round(x3,6)
            return x3,inter
        else:
            return None,None

#a,b = zoloto(-3,0,0.001,0)
#print(a,b)


def table(num = 0,a = 0,b = 0,x = 0,inter = 0,err = 0,eps = 0):
    """Печатает таблицу."""
    if err != -1:
        print('|{6:^5}|{0:^10.3f}|{1:^10.3f}|{2:^10.3f}|\
{3:^10.3e}|{4:^13}|{5:^16}|'.format(a,b,x,f(x),inter,err,num))


def main1_graf():
    """Главная процедура программы."""
    print('Метод золотого сечения.')
    print('Нахождение корней.')
    #xMas,fMas - хранят список корней координатно
    xMas = []
    fMas = []
    a = float(input('Введите а: '))
    b = float(input('Введите b: '))
    h = float(input('Введите шаг h: '))
    minter = int(input('Введите максимальное кол-во итераций: '))
    eps = float(input('Введите точность eps: '))
    print()
    print('|  №  |    x1    |    x2    |    x     |    f(x)  |  Итерации   \
|  Номер ошибки  |')                            
    x1 = a
    x2 = a+h
    k = 1
    kor = 0
    zero = 0
    #Цикл разбиения отрезка по h
    while x1 < b:
        if x2 > b:
            x2 = b
        if f(x2) == 0:
            zero = 1
            
        xk,inter = zoloto(x1,x2,eps,minter,0)
        err = 0
        #print(x1,x2,xk,f(xk),inter)
        #if f(xk) > 0 + 2*eps or f(xk) < -2*eps:
            #err = -1
        if f(x1) == 0 and zero == 1:
            xk = None
        #Проверка на ошибку по максимальному кол-ву интераций
        if inter != None:
            if inter > minter:
                err = 1
        #Проверка на наличие корня
        if xk != None:
            kor += 1
            table(k,x1,x2,xk,inter,err,eps)
        if xk != None:
            #Запись координаты корня для отображения в графике
            xMas.append(xk)
            fMas.append(f(xk))
        x1 , x2 = x2 , x2 + h
        if xk != None: 
            k += 1
    #Условие проверки на наличие корней
    if kor == 0:
        print('                Корней нет.')
    else:
        print('Типы ошибок: ')
        print('0 - все отлично.')
        print('1 - превышено кол-во итераций1.')
    makeGraph(a,b,xMas,fMas,f)
#main1_graf()
